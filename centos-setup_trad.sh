...
configure_shell() {
  # Install un invite de commande personnalise et des alias utiles.
  ...
  # Les utilisateurs existants pourrait aussi l'utiliser.
  if [ ! -z "${USERS}" ]
  then
    for USER in ${USERS}
    do
  ...
  # Ajoute des options systeme utiles pour Vim.
  echo 'Configuring Vim.'
  cat ${CWD}/${VERSION}/vim/vimrc > /etc/vimrc
  # Configure l'anglais commme language par defaut du systeme.
...
configure_repos() {
  # Active les dépôts [base], [updates] et [extra] avec une priorite de 1.
  ...
  # Active le dépôt [epel] avec une priorite de 10.
  ...
  # Configure les dépôts [elrepo] et [elrepo-kernel] sans les activer.
  ...
  # Active le dépôt [lynis] avec une priorite de 58.
...
configure_logs() {
  # Admin peut avoir acces aux journaux systeme
  if [ ! -z "${ADMIN}" ]
  then
...
strip_system() {
  # Supprime tous les paquets qui ne font pas partie du systeme de base.
...
# Verifie que le script est execute avec des droits administrateurs.
if [[ "${UID}" -ne 0 ]]
then
